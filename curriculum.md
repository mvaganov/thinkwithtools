...
<li> Basic ethics of Computer Programming<ul>
  ...
  <li> (possibly out-of-scope additional discussion) As of January 2026, LLMs are good at quickly generating top-down design of software (when prompted to do so). And LLMs are also good at generating examples of bottom-up implementation of software (as API tutorials, when prompted to do so). However, LLMs are bad at integrating bottom-up implementation into top-down design. If you ask an LLM to explain integration pain points (for integrating high-level design with low-level implementation), it will give some valid ideas, and probably be oblivious to significant practical software development roadblocks.
</ul>
